...
void sb_onHotkeyPressed(const char* keyword)
{
	...
	else if (strcmp(keyword, HOTKEY_PLAYLIST_START) == 0)
	{
		if (configDialog)
			configDialog->playlistStart();
	}
	else if (strcmp(keyword, HOTKEY_PLAYLIST_NEXT) == 0)
	{
		if (configDialog)
			configDialog->playlistNext();
	}
	else if (strcmp(keyword, HOTKEY_PLAYLIST_PREV) == 0)
	{
		if (configDialog)
